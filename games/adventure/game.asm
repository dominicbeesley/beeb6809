...
1		lda	,X+
		...
		; select device and page in chipset
		lda	#JIM_DEVNO_BLITTER
		sta	fred_JIM_DEVNO
		eora	fred_JIM_DEVNO
		bne	okblit
		BRK
		fcb	255, "No blitter", 0

okblit		ldx	#jim_page_DMAC
...
1		lda	#15
		...
		ldb	#8
		std	jim_DMAC_STRIDE_B		
		ldd	#640
		std	jim_DMAC_STRIDE_C
		std	jim_DMAC_STRIDE_D
...
dmac_copy	
		pshs	CC
		SEI
		clr	jim_DMAC_DMA_SEL		; always use cha 0
		stu	jim_DMAC_DMA_COUNT
		sta	jim_DMAC_DMA_SRC_ADDR
		stx	jim_DMAC_DMA_SRC_ADDR + 1
		stb	jim_DMAC_DMA_DEST_ADDR
