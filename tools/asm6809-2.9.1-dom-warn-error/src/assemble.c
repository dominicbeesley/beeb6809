...
static struct pseudo_op pseudo_ops[] = {
	...
	{ .name = "error", .handler = &pseudo_error },
	{ .name = "warn", .handler = &pseudo_warn },
};
...
static void pseudo_error(struct prog_line *line) {
	if (verify_num_args(line->args, 1, 1, "ERROR") < 0)
		return;

	struct node **arga = node_array_of(line->args);
	struct node *n = eval_string(arga[0]);
	if (!n) {
		error(error_type_syntax, "invalid argument to ERROR");
		return;
	}

	error(error_type_fatal, "%s", n->data.as_string);
}

/* WARN. issue a warning */
static void pseudo_warn(struct prog_line *line) {
	if (verify_num_args(line->args, 1, 1, "WARN") < 0)
		return;

	struct node **arga = node_array_of(line->args);
	struct node *n = eval_string(arga[0]);
	if (!n) {
		error(error_type_syntax, "invalid argument to WARN");
		return;
	}

	error(error_type_illegal, "%s", n->data.as_string);
}
...
static const char * include_rel(const char *filename) {

	struct prog_ctx *ctx = prog_ctx_stack->data;
	assert(ctx != NULL);
	struct prog *prog = ctx->prog;
	assert(prog != NULL);
	const char *curfile = prog->name;



	if (asm6809_options.relative_includes && curfile && curfile[0] != '/' 
		&& !(curfile[0] && filename[1] == ':' && filename[2] == '\\')) {
		// find final / or \\ in current filename and append our file to that unless this filename
		// starts eith / or ?:\\
		
		char *lastsl = strrchr(curfile, '\\');
		char *lastsl2 = strrchr(curfile, '/');
		int l = 0;
		if (lastsl && lastsl > lastsl2)
			l = 1 + lastsl - curfile;
		else if (lastsl2)
			l = 1 + lastsl2 - curfile;

		if (l>0) {
			char *ret = malloc(1 + l + strlen(filename));
			if (!ret)
				error(error_type_fatal, "out of memory");
			strncpy(ret, curfile, l);
			strcpy(ret+l, filename);
			return ret;
		} else {
			return filename;
		}
...
static void pseudo_include(struct prog_line *line) {
	...
	struct prog *file = prog_new_file(include_rel(arga[0]->data.as_string));
	if (!file)
		return;
...
static void pseudo_includebin(struct prog_line *line) {
	...
	const char *relfile = include_rel(arga[0]->data.as_string);

	FILE *f = fopen(relfile, "rb");
	if (!f) {
		error(error_type_fatal, "file not found: %s", relfile);
		return;
	}
	int c;
	while ((c = fgetc(f)) != EOF) {
