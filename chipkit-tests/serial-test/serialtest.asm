* QUICK SERIAL PORT ECHO TEST
*
RAM_START       EQU     $100        ; ; START OF MONITOR RAM
RAM_END			EQU		$200
ROM_START       EQU     $8000       ; ; START OF MONITOR CODE
HARD_VECT       EQU     $F7F0       ; ; START OF HARDWARE VECTORS

*============================================================================
*  Equates for memory mapped 16450 serial port on Heng's ROM emulator board
*;*        IFEQ ROMEM,1
*
S16450  equ     $FE78           base of 16450 UART
...
COMBUF_SIZE		 EQU	$80 - 3 		; allow for FN, LEN and CHR
COMBUF			 EQU	$100
...
SCREEN_BASEx8   EQU     0
SHEILA_CRTC     EQU     $FE00           ; CRTC controller
SHEILA_VID_ULA  EQU     $FE20           ; Video ULA 


                ; setup CRTC
                LDB     #$B
                LDX     #mostbl_VDU_6845_mode_012
crtcsetlp1      STB     SHEILA_CRTC
                LDA     B,X
                STA     SHEILA_CRTC + 1
                DECB
                BPL     crtcsetlp1
...
*  access baud generator, no parity, 1 stop bit, 8 data bits
		...
		STA     S16450+FCR			; no FIFOs

*
*  fixed baud rate of 19200:  crystal is 3.686400 Mhz.
*  Divisor is 4000000/(16*baud)
BAUD_CLOCK_IN	EQU	7372800
BAUD_RATE		EQU 19200
BAUD_WORD		EQU BAUD_CLOCK_IN/(BAUD_RATE*16)
        LDA     #BAUD_WORD%256
        STA     S16450+RXR              ; lsb
        LDA     #BAUD_WORD/256
...
*
GETCHAR
GC10    LDA     SER_STATUS     ; READ DEVICE STATUS
        ANDA    #RXRDY
